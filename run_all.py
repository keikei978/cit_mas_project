...
def run_experiments() -> list[str]:
    ...
    for script_name in EXPERIMENT_SCRIPTS:
        ...
        env["PYTHONIOENCODING"] = "utf-8"  # Windows での文字化け防止
...
def _git(args: list[str], cwd: Path = None, check: bool = True,
    ...
    """git コマンドのラッパー。Windows (CP932) でも文字化けしないよう utf-8 固定。"""
    return subprocess.run(
        ...
        text=True,
        encoding='utf-8',   # ★ Windows CP932 対策
        errors='replace',   # ★ デコード失敗時も継続（クラッシュしない）
    )
...
def git_push_results():
    ...
    try:
        ...
        # 5. 元のブランチへ戻る（-f で未追跡ファイルの衝突を強制解決）
        # orphan 作成後はディスク上に元ブランチのファイルが残るため -f が必要
        _git(["checkout", "-f", current_branch])
